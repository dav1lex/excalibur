<div class="row mb-4">
    <div class="col-md-12">
        <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
            <h1 class="display-5 mb-0">Edit Auction</h1>
                    ...
                    <form action="<?= BASE_URL ?>auctions/update/<?= $auction['id'] ?>" method="post"
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="title" name="title"
                                value="<?= htmlspecialchars($auction['title']) ?>" required>
                        </div>
                            ...
                            <?php if (!empty($auction['image_path'])): ?>
                                <div class="mb-2">
                                    <img src="<?= BASE_URL . htmlspecialchars($auction['image_path']) ?>"
                                        alt="Current auction image" class="img-thumbnail" style="max-height: 200px;">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" id="status" name="status">
                                <option value="draft" <?= $auction['status'] === 'draft' ? 'selected' : '' ?>>Draft
                                </option>
                                <option value="upcoming" <?= $auction['status'] === 'upcoming' ? 'selected' : '' ?>>
                                    Upcoming</option>
                                <option value="live" <?= $auction['status'] === 'live' ? 'selected' : '' ?>>Live</option>
                                <option value="ended" <?= $auction['status'] === 'ended' ? 'selected' : '' ?>>Ended
                                </option>
                            </select>
                        </div>
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        ...
                        <button type="button" class="btn btn-danger" data-bs-toggle="modal"
                            data-bs-target="#deleteAuctionModal<?= $auction['id'] ?>">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Are you sure you want to delete the auction "<?= htmlspecialchars($auction['title']) ?>"? This
                    action cannot be undone. Lots associated with this auction will also be deleted.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <a href="<?= BASE_URL ?>auctions/delete/<?= $auction['id'] ?>" class="btn btn-danger">Delete
                        Auction</a>
                </div>
            </div>
        </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($lot['image_path'])): ?>
                                            <img src="<?= BASE_URL . htmlspecialchars($lot['image_path']) ?>" alt="Lot Image"
